...
export function pickBestRawgMatch(query: string, results: RAWGGame[]): RAWGGame | null {
...
export class RAWGAPI {
  ...
  async searchGames(query: string, page = 1): Promise<RAWGSearchResponse> {
    ...
    return getJson<RAWGSearchResponse>(`${RAWG_API_BASE}/games?${params.toString()}`);
  }

  async getGameDetails(id: number): Promise<RAWGGame> {
    return getJson<RAWGGame>(`${RAWG_API_BASE}/games/${id}?key=${encodeURIComponent(this.apiKey)}`);
  }

  async getGameBySlug(slug: string): Promise<RAWGGame> {
    return getJson<RAWGGame>(`${RAWG_API_BASE}/games/${slug}?key=${encodeURIComponent(this.apiKey)}`);
  }

  /**
   * Search + best title match + details (community rating + metacritic).
   */
  async resolveRatingsForTitle(title: string): Promise<{
    ...
    const rawg = typeof details.rating === 'number' && details.rating > 0 ? details.rating : null;
    const metacritic =
