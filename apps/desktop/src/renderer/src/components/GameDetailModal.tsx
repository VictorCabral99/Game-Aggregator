...
function sourceLabel(source: GameSource): string {
  ...
  if (source.platform === 'local' && source.executable) {
    const base = source.executable.replace(/^.*[/\\]/, '');
    return `${platform} · ${base}`;
  }
  if (source.externalId) {
    const id = source.platform === 'steam' ? source.externalId : shortId(source.externalId);
    return `${platform} · ${id}`;
  }
  return platform;
}
...
export default function GameDetailModal({
  ...
  // Foco no Instalar/Iniciar ao abrir — não no link SteamDB / Atualizar
  useEffect(() => {
  ...
  return (
    <div className="modal-backdrop modal-backdrop--detail" onClick={onClose}>
      <div
            ...
            <div>
              ...
              <dd>
                {game.sources.length > 0
                  ? game.sources.map((s) => PLATFORM_LABELS[s.platform]).join(' · ')
                  : 'Nenhuma'}
              </dd>
            </div>
            ...
            <div className="ratings">
              <div className="ratings__head">
                ...
                {rating?.updatedAt && (
                  ...
                  >
                    {isStale(rating.updatedAt)
                      ? 'desatualizadas'
                      : `atualizadas ${dateLabel(rating.updatedAt)}`}
                  </span>
                )}
                {onSyncRating && (
              ...
              {!rating || rating.sources.every((s) => s.score === null) ? (
                ...
                <div className="ratings__grid">
                  {rating.sources.map((s) => {
                    ...
                    const display =
                      s.source === 'rawg' && s.score <= 5
                        ? Math.round(s.score * 20 * 10) / 10
                        : s.score;
                    return (
              ...
              {game.sources.map((source) => {
                ...
                return (
                  <div
                    ...
                    title={
                      source.externalId
                        ? `${PLATFORM_LABELS[source.platform]} · ${source.externalId}`
                  ...
                  >
                    <span className="source-row__label">
                      <span className="source-row__name">{sourceLabel(source)}</span>
        ...
        <div className="modal__chrome">
          ...
          {preferredNeedsInstall && (
            <p className="hint hint--compact">
              Abre o cliente da loja. Depois sincronize em Lojas para marcar instalado.
            </p>
          )}

          <div className="modal__footer-danger">
            {confirmingRemove ? (
              <>
                <span className="modal__footer-danger-hint">
                  Remove só da biblioteca — não apaga arquivos.
                </span>
                <button type="button" className="danger" onClick={() => void onRemove()}>
                  Confirmar remoção
                </button>
                <button type="button" className="ghost" onClick={() => setConfirmingRemove(false)}>
                  Cancelar
                </button>
              </>
            ) : (
